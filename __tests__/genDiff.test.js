...
test('should compare JSON files', () => {
  const actual = genDiff(path.join(fixturesPath, 'json', 'file1.json'), path.join(fixturesPath, 'json', 'file2.json'))
  const expected = fs.readFileSync(path.join(fixturesPath, 'json', 'expected.txt'), 'utf-8')
